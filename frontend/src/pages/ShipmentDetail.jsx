  ...
  const handlePrint = () => {
    ...
    pw.document.write(`<!DOCTYPE html><html><head><title>${shipment.refNo} - PAS Freight</title><style>
      *{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial;padding:40px;color:#1a1a1a;max-width:900px;margin:auto}
      ...
      <div class="item"><label>Consignee</label><span>${ff.consigneeName||'—'}</span></div><div class="item"><label>Shipper</label><span>${ff.shipperName||'—'}</span></div><div class="item"><label>From</label><span>${ff.fromLocation||'—'}</span></div><div class="item"><label>To</label><span>${ff.toLocation||'—'}</span></div><div class="item"><label>Agent</label><span>${ff.agent||'—'}</span></div><div class="item"><label>Packages</label><span>${ff.noOfPackages||'—'}</span></div><div class="item"><label>Selling Rate</label><span>${ff.sellingRate?'$'+ff.sellingRate:'—'}</span></div><div class="item"><label>Weight</label><span>${ff.weight?ff.weight+' kg':'—'}</span></div><div class="item"><label>Booking Date</label><span>${fmt(ff.bookingDate)}</span></div><div class="item"><label>ETD</label><span>${fmt(ff.etd)}</span></div><div class="item"><label>ETA</label><span>${fmt(ff.eta)}</span></div><div class="item"><label>MAWB</label><span>${ff.mawb||'—'}</span></div><div class="item"><label>HAWB</label><span>${ff.hawb||'—'}</span></div><div class="item"><label>AWB Date</label><span>${fmt(ff.awbDate)}</span></div></div></div>
