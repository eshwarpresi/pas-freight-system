...
function InlineField({ value, onSave, type = 'text', placeholder = '—', className = '', options = null }) {
  ...
  if (editing) {
    if (options) return <select ref={inputRef} value={val} onChange={e => setVal(e.target.value)} onBlur={save} className="border border-blue-300 rounded px-2 py-1 text-sm bg-white focus:ring-2 focus:ring-blue-500 w-full"><option value="">Select...</option>{options.map(o => <option key={o} value={o}>{o}</option>)}</select>
    return <input ref={inputRef} type={type} value={val} onChange={e => setVal(e.target.value)} onBlur={save} onKeyDown={e => { if (e.key === 'Enter') save(); if (e.key === 'Escape') { setVal(value || ''); setEditing(false) } }} className="border border-blue-300 rounded px-2 py-1 text-sm bg-white focus:ring-2 focus:ring-blue-500 w-full" step={type === 'number' ? '0.01' : undefined} />
  }
  return <div onClick={() => setEditing(true)} className={`cursor-pointer group flex items-center gap-1 ${className}`}><span className={value ? '' : 'text-gray-400 italic'}>{value || placeholder}</span><Pencil size={10} className="text-gray-300 group-hover:text-blue-500 opacity-0 group-hover:opacity-100" /></div>
}
...
export default function ShipmentDetail() {
  ...
  const { data: shipment, isLoading, refetch } = useQuery({
    queryKey: ['shipment', id], queryFn: async () => { const r = await api.get(`/freight/shipments/${id}`); return r.data.data }
  })

  const updateMutation = useMutation({
    mutationFn: async ({ section, data }) => {
      const eps = {
        rates:{u:`/freight/shipments/${id}/rates`,m:'put'},nomination:{u:`/freight/shipments/${id}/nomination`,m:'put'},booking:{u:`/freight/shipments/${id}/booking`,m:'put'},schedule:{u:`/freight/shipments/${id}/schedule`,m:'put'},awb:{u:`/freight/shipments/${id}/awb`,m:'put'},checklist:{u:`/cha/shipments/${id}/checklist`,m:'put'},boe:{u:`/cha/shipments/${id}/boe`,m:'put'},do:{u:`/cha/shipments/${id}/do-collection`,m:'put'},ooc:{u:`/cha/shipments/${id}/ooc`,m:'put'},gatepass:{u:`/cha/shipments/${id}/gate-pass`,m:'put'},pod:{u:`/cha/shipments/${id}/pod`,m:'put'},invoice:{u:`/accounts/shipments/${id}/invoice`,m:'put'},invoiceSend:{u:`/accounts/shipments/${id}/invoice-send`,m:'put'},stage:{u:`/freight/shipments/${id}/stage`,m:'put'},remarks:{u:`/freight/shipments/${id}/remarks`,m:'put'}
      }
  ...
  const handlePrint = () => {
    ...
    pw.document.write(`<!DOCTYPE html><html><head><title>${shipment.refNo} - PAS Freight</title><style>
      *{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;padding:40px;color:#1a1a1a;max-width:900px;margin:auto}
      ...
      .header h1{color:#1e40af;font-size:22px}.header p{color:#666;font-size:12px}
      .ref-box{display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;padding:10px 15px;background:#f0f4ff;border-radius:8px}
      .ref{font-size:20px;font-weight:bold;color:#1e40af}.stage{font-size:12px;padding:3px 12px;border-radius:20px;background:#dbeafe;color:#1e40af;font-weight:bold}
      .section{margin-bottom:20px}.section h2{font-size:14px;color:#1e40af;border-bottom:2px solid #e5e7eb;padding-bottom:5px;margin-bottom:10px;text-transform:uppercase;letter-spacing:1px}
      ...
      <div class="item"><label>Consignee</label><span>${ff.consigneeName||'—'}</span></div><div class="item"><label>Shipper</label><span>${ff.shipperName||'—'}</span></div><div class="item"><label>Agent</label><span>${ff.agent||'—'}</span></div><div class="item"><label>Packages</label><span>${ff.noOfPackages||'—'}</span></div><div class="item"><label>Selling Rate</label><span>${ff.sellingRate?'$'+ff.sellingRate:'—'}</span></div><div class="item"><label>Weight</label><span>${ff.weight?ff.weight+' kg':'—'}</span></div><div class="item"><label>Booking Date</label><span>${fmt(ff.bookingDate)}</span></div><div class="item"><label>ETD</label><span>${fmt(ff.etd)}</span></div><div class="item"><label>ETA</label><span>${fmt(ff.eta)}</span></div><div class="item"><label>MAWB</label><span>${ff.mawb||'—'}</span></div><div class="item"><label>HAWB</label><span>${ff.hawb||'—'}</span></div><div class="item"><label>AWB Date</label><span>${fmt(ff.awbDate)}</span></div></div></div>
      <div class="section"><h2>🛃 Customs Clearance (CHA)</h2><div class="grid">
      ...
      <div class="section"><h2>💰 Accounts</h2><div class="grid">
      <div class="item"><label>Invoice No</label><span>${acc.invoiceNumber||'—'}</span></div><div class="item"><label>Invoice Date</label><span>${fmt(acc.invoiceDate)}</span></div><div class="item"><label>Sending Date</label><span>${fmt(acc.sendingDate)}</span></div></div></div>
      ${shipment.remarks?`<div class="remarks-box"><strong>Remarks:</strong> ${shipment.remarks}</div>`:''}
      <div class="footer">© ${new Date().getFullYear()} PAS Freight Services Pvt Ltd | Generated by PAS Freight Management System</div>
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div><Link to="/" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 mb-3"><ArrowLeft size={15} />Back</Link>
        <div className="bg-white rounded-xl border p-5 shadow-sm space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-4 pt-3 border-t border-gray-100"><div className="flex items-center gap-2"><Flag size={14} className="text-gray-400" /><span className="text-xs text-gray-500 font-medium">Stage:</span><InlineField value={shipment.shipmentStage} options={STAGE_OPTIONS} onSave={v => updateMutation.mutate({ section: 'stage', data: { shipmentStage: v } })} className={shipment.shipmentStage ? `px-2 py-0.5 rounded-full text-xs font-medium ${STAGE_COLORS[shipment.shipmentStage] || ''}` : ''} placeholder="Set stage" /></div><div className="flex items-center gap-2 flex-1"><MessageSquare size={14} className="text-gray-400" /><span className="text-xs text-gray-500 font-medium">Remarks:</span><InlineField value={shipment.remarks} onSave={v => updateMutation.mutate({ section: 'remarks', data: { remarks: v } })} placeholder="Add remarks..." className="flex-1" /></div></div></div></div>
      ...
      <div className="bg-white rounded-xl border p-6">
        {activeTab==='freight'&&<div className="space-y-4"><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"><C icon={User} l="Consignee" v={ff.consigneeName}/><C icon={User} l="Shipper" v={ff.shipperName}/><C icon={Anchor} l="Agent" v={ff.agent}/><C icon={Package} l="Packages" v={ff.noOfPackages}/></div><Section title="Rates" icon={DollarSign}><div className="grid grid-cols-2 gap-3"><Field label="Selling Rate ($)" value={ff.sellingRate} onSave={v => updateMutation.mutate({ section: 'rates', data: { sellingRate: v } })} type="number" /><Field label="Weight (kg)" value={ff.weight} onSave={v => updateMutation.mutate({ section: 'rates', data: { weight: v } })} type="number" /></div></Section><Section title="Nomination" icon={Calendar}><Field label="Nomination Date" value={Fmt(ff.nominationDate)} onSave={v => updateMutation.mutate({ section: 'nomination', data: { nominationDate: v } })} type="date" /></Section><Section title="Booking" icon={Calendar}><Field label="Booking Date" value={Fmt(ff.bookingDate)} onSave={v => updateMutation.mutate({ section: 'booking', data: { bookingDate: v } })} type="date" /></Section><Section title="Schedule" icon={Plane}><div className="grid grid-cols-2 gap-3"><Field label="ETD" value={Fmt(ff.etd)} onSave={v => updateMutation.mutate({ section: 'schedule', data: { etd: v } })} type="date" /><Field label="ETA" value={Fmt(ff.eta)} onSave={v => updateMutation.mutate({ section: 'schedule', data: { eta: v } })} type="date" /></div></Section><Section title="AWB Details" icon={Barcode}><div className="grid grid-cols-3 gap-3"><Field label="MAWB" value={ff.mawb} onSave={v => updateMutation.mutate({ section: 'awb', data: { mawb: v } })} /><Field label="HAWB" value={ff.hawb} onSave={v => updateMutation.mutate({ section: 'awb', data: { hawb: v } })} /><Field label="AWB Date" value={Fmt(ff.awbDate)} onSave={v => updateMutation.mutate({ section: 'awb', data: { awbDate: v } })} type="date" /></div></Section></div>}
