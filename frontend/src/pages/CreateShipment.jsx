...
import { 
  ...
  FileCheck, ArrowUpDown, Barcode, Weight, Info, Pencil, Eye, Scale, Mail, Truck, MapPin, ClipboardList
} from 'lucide-react'
...
export default function CreateShipment() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      const shipmentTypeVal = isDORelease ? 'DO Release' : isTransport ? 'Transport' : (isCHA ? 'CHA Only' : (formData.mode || ''))
      ...
      if (isEditMode) {
        ...
        addToast(isDORelease ? 'DO Release created!' : isTransport ? 'Transport shipment created!' : isCHA ? 'CHA Bill created successfully!' : 'Shipment created successfully!', 'success')
        if (isTransport) {
  ...
  const focusRing = isDORelease ? 'focus:ring-teal-500 focus:border-teal-500' : isTransport ? 'focus:ring-sky-500 focus:border-sky-500' : isCHA ? (isCHAExport ? 'focus:ring-amber-500 focus:border-amber-500' : 'focus:ring-emerald-500 focus:border-emerald-500') : 'focus:ring-indigo-500 focus:border-indigo-500'
  const accentText = isDORelease ? 'text-teal-400' : isTransport ? 'text-sky-400' : isCHAExport ? 'text-amber-400' : isCHA ? 'text-emerald-400' : 'text-indigo-400'
  const inputClass = `w-full pl-9 pr-3 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 ${focusRing}`
  const accentBg = isDORelease ? 'from-teal-400 to-emerald-500' : isTransport ? 'from-sky-400 to-blue-500' : isCHAExport ? 'from-amber-400 to-orange-500' : isCHA ? 'from-emerald-400 to-green-500' : 'from-indigo-500 to-blue-600'
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center gap-3">
          <div className={`w-12 h-12 bg-gradient-to-br ${accentBg} rounded-xl flex items-center justify-center shadow-lg`}>
            {isEditMode ? <Pencil size={22} className="text-white" /> : isDORelease ? <ClipboardList size={22} className="text-white" /> : isTransport ? <Truck size={22} className="text-white" /> : isCHA ? <FileCheck size={22} className="text-white" /> : <Ship size={22} className="text-white" />}
          </div>
          <div className="flex-1">
            <h2 className="text-2xl font-bold bg-gradient-to-r from-indigo-600 to-blue-600 bg-clip-text text-transparent">
              {isEditMode ? `Edit: ${formData.refNo}` : isDORelease ? 'New DO Release' : isTransport ? 'New Transport Shipment' : isCHAExport ? 'New CHA Bill Export' : isCHA ? 'New CHA Bill Import' : 'New Freight Shipment'}
            </h2>
          </div>
