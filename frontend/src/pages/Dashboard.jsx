...
export default function Dashboard() {
  ...
  const getStatusBadge = (s) => {
    const b = {
      'ENQUIRY':'bg-gradient-to-r from-amber-400 to-amber-300 text-amber-900 ring-amber-300',
      'RATES_ADDED':'bg-gradient-to-r from-sky-400 to-sky-300 text-sky-900 ring-sky-300',
      'NOMINATED':'bg-gradient-to-r from-violet-400 to-violet-300 text-violet-900 ring-violet-300',
      'BOOKED':'bg-gradient-to-r from-indigo-400 to-indigo-300 text-indigo-900 ring-indigo-300',
      'SCHEDULED':'bg-gradient-to-r from-cyan-400 to-cyan-300 text-cyan-900 ring-cyan-300',
      'AWB_GENERATED':'bg-gradient-to-r from-teal-400 to-teal-300 text-teal-900 ring-teal-300',
      'CHECKLIST_APPROVED':'bg-gradient-to-r from-emerald-400 to-emerald-300 text-emerald-900 ring-emerald-300',
      'BOE_FILED':'bg-gradient-to-r from-lime-400 to-lime-300 text-lime-900 ring-lime-300',
      'DO_COLLECTED':'bg-gradient-to-r from-green-400 to-green-300 text-green-900 ring-green-300',
      'OOC_DONE':'bg-gradient-to-r from-sky-500 to-sky-400 text-sky-900 ring-sky-400',
      'GATE_PASS':'bg-gradient-to-r from-purple-400 to-purple-300 text-purple-900 ring-purple-300',
      'DELIVERED':'bg-gradient-to-r from-emerald-500 to-emerald-400 text-white ring-emerald-400',
      'INVOICE_GENERATED':'bg-gradient-to-r from-orange-400 to-orange-300 text-orange-900 ring-orange-300',
      'INVOICE_SENT':'bg-gradient-to-r from-rose-400 to-rose-300 text-rose-900 ring-rose-300',
      'COMPLETED':'bg-gradient-to-r from-gray-400 to-gray-300 text-gray-800 ring-gray-300'
    }
    return b[s]||'bg-gradient-to-r from-gray-400 to-gray-300 text-gray-700 ring-gray-300'
  }

  const getModeBadge = (type) => {
    if (type === 'CHA Only') return 'bg-gradient-to-r from-emerald-500 to-green-500 text-white ring-green-400'
    if (!type) return 'bg-gradient-to-r from-gray-400 to-gray-300 text-gray-600 ring-gray-300'
    return 'bg-gradient-to-r from-blue-500 to-indigo-500 text-white ring-blue-400'
  }

  const getImportExportBadge = (val) => {
    if (val === 'Import') return 'bg-gradient-to-r from-violet-500 to-purple-500 text-white ring-purple-400'
    if (val === 'Export') return 'bg-gradient-to-r from-orange-500 to-amber-500 text-white ring-amber-400'
    return 'bg-gradient-to-r from-gray-400 to-gray-300 text-gray-600 ring-gray-300'
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-[28px] font-bold bg-gradient-to-r from-indigo-600 to-blue-600 bg-clip-text text-transparent tracking-tight">
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          <div className="flex bg-gradient-to-r from-indigo-50 to-blue-50 backdrop-blur rounded-lg p-0.5 border border-indigo-200/50">
            <button onClick={()=>updateShipmentTypeFilter('')} className={`px-3 py-2 rounded-md text-xs font-semibold transition-all duration-200 ${!shipmentTypeFilter?'bg-white text-indigo-700 shadow-sm':'text-gray-500 hover:text-gray-700'}`}>All</button>
            <button onClick={()=>updateShipmentTypeFilter('FULL_SHIPMENT')} className={`px-3 py-2 rounded-md text-xs font-semibold transition-all duration-200 ${shipmentTypeFilter==='FULL_SHIPMENT'?'bg-white text-indigo-700 shadow-sm':'text-gray-500 hover:text-gray-700'}`}>Freight</button>
            <button onClick={()=>updateShipmentTypeFilter('CHA_ONLY')} className={`px-3 py-2 rounded-md text-xs font-semibold transition-all duration-200 flex items-center gap-1 ${shipmentTypeFilter==='CHA_ONLY'?'bg-white text-emerald-700 shadow-sm':'text-gray-500 hover:text-gray-700'}`}><FileCheck size={12}/>CHA Only</button>
          </div>
          ...
          <Link to="/create" className="inline-flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-indigo-600 to-blue-600 text-white rounded-lg hover:from-indigo-700 hover:to-blue-700 text-xs font-semibold transition-all duration-200 shadow-lg shadow-indigo-200">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {statCards.map((stat, i) => {
          ...
          return (
            <div key={i} className="relative overflow-hidden bg-white/80 backdrop-blur rounded-xl p-4 border border-white/50 hover:scale-[1.02] transition-all duration-300 shadow-lg group">
              ...
              <div className="relative">
                ...
                <p className="text-[11px] text-gray-500 mt-0.5 font-semibold">{stat.label}</p>
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-blue-50 backdrop-blur rounded-xl border border-indigo-200/50 p-4">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold text-indigo-500 uppercase tracking-wider">Delivery Progress</span>
            <Info size={12} className="text-indigo-400" />
          </div>
          <span className="text-xs font-bold text-indigo-700">{analytics.deliveryRate}%</span>
        </div>
        <div className="w-full bg-gray-200/50 rounded-full h-2 overflow-hidden">
          <div className="h-2 rounded-full bg-gradient-to-r from-emerald-400 to-teal-500 transition-all duration-700 ease-out shadow-inner" style={{ width: `${analytics.deliveryRate}%` }} />
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2.5">
        <div className="relative flex-1 w-full">
          ...
          <input type="text" placeholder="Search by Ref No, Consignee, HAWB, BOE, Invoice..." value={search} onChange={e=>updateSearch(e.target.value)}
            className="w-full pl-9 pr-9 py-2.5 bg-white/80 backdrop-blur border border-indigo-200/50 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 text-sm transition-all" />
        ...
        <div className="flex flex-wrap gap-2 p-3.5 bg-gradient-to-r from-indigo-50 to-blue-50 backdrop-blur rounded-xl border border-indigo-200/50">
          ...
          {quickFilters.map(f=>{const I=f.i;const a=statusFilter===f.v;return <button key={f.v} onClick={()=>updateStatus(a?'':f.v)}
            className={`px-3 py-1.5 rounded-md text-[11px] font-semibold transition-all duration-200 flex items-center gap-1.5 ${
              a?'bg-gradient-to-r from-indigo-600 to-blue-600 text-white shadow-lg':'bg-white/80 text-gray-600 hover:bg-white'}`}><I size={12}/>{f.l}{a&&<X size={11}/>}</button>})}
        </div>
      )}
      ...
      {selected.length > 0 && (
        <div className="bg-gradient-to-r from-indigo-50 to-blue-50 backdrop-blur border border-indigo-300/50 rounded-xl px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            {!showArchived && <button onClick={()=>bulkArchiveMutation.mutate(selected)} className="px-3.5 py-1.5 bg-gradient-to-r from-indigo-600 to-blue-600 text-white rounded-lg text-xs font-semibold hover:from-indigo-700 hover:to-blue-700 flex items-center gap-1.5 transition-all shadow-lg"><Archive size={13}/> Archive</button>}
            <button onClick={()=>setSelected([])} className="px-3.5 py-1.5 border border-indigo-300 text-indigo-700 rounded-lg text-xs font-semibold hover:bg-indigo-100 transition-all">Clear</button>
          </div>
        </div>
      )}
      ...
      {isError && (
        <div className="bg-white/80 backdrop-blur rounded-xl border border-red-200/50 p-16 text-center">
          ...
          <button onClick={()=>refetch()} className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-indigo-600 to-blue-600 text-white rounded-lg text-sm font-semibold hover:from-indigo-700 hover:to-blue-700 transition-all shadow-lg"><RefreshCw size={14}/> Retry</button>
        </div>
      )}
      ...
      {!isError && isEmpty && !hasFilters && showArchived && (
        <div className="bg-white/80 backdrop-blur rounded-xl border border-gray-200/50 p-16 text-center">
          <div className="w-16 h-16 bg-gradient-to-br from-gray-400 to-slate-500 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg"><ArchiveIcon size={28} className="text-white"/></div>
        ...
        <>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gradient-to-r from-indigo-50 to-blue-50">
                    ...
                    <th className="text-left px-3 py-3 text-[11px] font-semibold text-indigo-500 uppercase tracking-wider">Date</th>
                    <th className="text-right pr-4 py-3 text-[11px] font-semibold text-indigo-500 uppercase tracking-wider">Actions</th>
                  </tr>
                </thead>
          ...
          <div className="bg-white/80 backdrop-blur rounded-xl border border-indigo-100/50 px-4 py-3 flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-0.5">
              <button onClick={()=>setPage(1)} disabled={page===1} className="p-1.5 rounded-md hover:bg-indigo-50 disabled:opacity-30 disabled:cursor-not-allowed transition-all"><ChevronsLeft size={14}/></button>
              <button onClick={()=>setPage(p=>Math.max(1,p-1))} disabled={page===1} className="p-1.5 rounded-md hover:bg-indigo-50 disabled:opacity-30 disabled:cursor-not-allowed transition-all"><ChevronLeft size={14}/></button>
              {generatePageNumbers(page,totalPages).map((p,i)=>p==='...'?<span key={i} className="px-1.5 text-gray-400 text-xs">...</span>:
                <button key={p} onClick={()=>setPage(p)} className={`w-8 h-8 rounded-md text-[11px] font-semibold transition-all ${page===p?'bg-gradient-to-r from-indigo-600 to-blue-600 text-white shadow-lg':'text-indigo-600 hover:bg-indigo-50'}`}>{p}</button>)}
              <button onClick={()=>setPage(p=>Math.min(totalPages,p+1))} disabled={page===totalPages||totalPages===0} className="p-1.5 rounded-md hover:bg-indigo-50 disabled:opacity-30 disabled:cursor-not-allowed transition-all"><ChevronRight size={14}/></button>
              <button onClick={()=>setPage(totalPages)} disabled={page===totalPages||totalPages===0} className="p-1.5 rounded-md hover:bg-indigo-50 disabled:opacity-30 disabled:cursor-not-allowed transition-all"><ChevronsRight size={14}/></button>
            </div>
          </div>
        </>
      )}
