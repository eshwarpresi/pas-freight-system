...
async function exportShipmentsToExcel(shipments, res) {
  ...
  const STAGE_FONT_COLORS = { 'Draft': '6B7280', 'Created': '1E40AF', 'Confirmed': '92400E', 'Booked': '6D28D9', 'Scheduled': '155E75', 'In Progress': 'A16207', 'Completed': '166534', 'Cancelled': 'DC2626', 'On Hold': 'C2410C' };

  const lastColLetter = 'AB'; // 28 columns (A-AB)
  const colCount = 28;
  ...
  ws.insertRow(1, ['']); ws.mergeCells(`A1:${lastColLetter}1`); ws.getRow(1).height = 55;
  ws.getCell('A1').value = '🚢  PAS FREIGHT SERVICES PVT LTD';
  ws.getCell('A1').font = { name: 'Calibri', size: 24, bold: true, color: { argb: '1E40AF' } };
  ws.getCell('A1').alignment = { horizontal: 'center', vertical: 'middle' };
  ws.getCell('A1').fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'F0F4FF' } };

  ws.insertRow(2, ['']); ws.mergeCells(`A2:${lastColLetter}2`); ws.getRow(2).height = 24;
  ws.getCell('A2').value = 'SHIPMENT REPORT';
  ws.getCell('A2').font = { name: 'Calibri', size: 14, bold: true, color: { argb: '475569' } };
  ws.getCell('A2').alignment = { horizontal: 'center' };

  ws.insertRow(3, ['']); ws.mergeCells(`A3:${lastColLetter}3`); ws.getRow(3).height = 22;
  ws.getCell('A3').value = `Generated: ${new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}  |  Total: ${shipments.length} shipments`;
  ws.getCell('A3').font = { name: 'Calibri', size: 10, color: { argb: '64748B' } }; ws.getCell('A3').alignment = { horizontal: 'center' };

  ws.insertRow(4, ['']); ws.getRow(4).height = 6;

  const headerRow = ws.getRow(5); headerRow.height = 34;
  columns.forEach((col, i) => {
    const cell = headerRow.getCell(i + 1); cell.value = col.header;
    const isStage = col.key === 'shipmentStage'; const isRemarks = col.key === 'remarks';
    const isFromTo = col.key === 'fromLocation' || col.key === 'toLocation';
    cell.font = { name: 'Calibri', size: 9, bold: true, color: { argb: 'FFFFFF' } };
    cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: isStage ? '7C3AED' : isRemarks ? '0D9488' : isFromTo ? '0891B2' : '1E40AF' } };
  ...
  ws.dataValidations.add(`C6:C${5 + shipments.length + 500}`, { type: 'list', allowBlank: true, formulae: [`"${STAGE_OPTIONS.join(',')}"`], showErrorMessage: true, errorTitle: 'Invalid Stage', error: 'Select a valid Stage.' });

  const statusRowColors = { 'ENQUIRY': 'FFF7ED', 'RATES_ADDED': 'F0F9FF', 'NOMINATED': 'F5F3FF', 'BOOKED': 'EEF2FF', 'SCHEDULED': 'ECFEFF', 'AWB_GENERATED': 'F0FDFA', 'CHECKLIST_APPROVED': 'ECFDF5', 'BOE_FILED': 'F7FEE7', 'DO_COLLECTED': 'F0FDF4', 'OOC_DONE': 'EFF6FF', 'GATE_PASS': 'F5F3FF', 'DELIVERED': 'DCFCE7', 'INVOICE_GENERATED': 'FFF7ED', 'INVOICE_SENT': 'FFF1F2', 'COMPLETED': 'F3F4F6' };
  ...
  shipments.forEach((s) => {
    ...
    const row = ws.addRow({
      ...
      consignee: ff.consigneeName || '', shipper: ff.shipperName || '',
      fromLocation: ff.fromLocation || '', toLocation: ff.toLocation || '',
      agent: ff.agent || '', packages: ff.noOfPackages || '', weight: ff.weight || '',
    ...
    row.alignment = { horizontal: 'center', vertical: 'middle' }; row.font = { name: 'Calibri', size: 9 };
    row.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: statusRowColors[s.currentStatus] || 'FFFFFF' } };
    const stageCell = row.getCell(3);
    if (s.shipmentStage && STAGE_COLORS[s.shipmentStage]) { stageCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: STAGE_COLORS[s.shipmentStage] } }; stageCell.font = { name: 'Calibri', size: 9, bold: true, color: { argb: STAGE_FONT_COLORS[s.shipmentStage] || '374151' } }; }
    const remCell = row.getCell(28); remCell.alignment = { horizontal: 'left', vertical: 'middle', wrapText: true };
    row.eachCell(cell => { cell.border = { top: { style: 'thin', color: { argb: 'E2E8F0' } }, bottom: { style: 'thin', color: { argb: 'E2E8F0' } }, left: { style: 'thin', color: { argb: 'E2E8F0' } }, right: { style: 'thin', color: { argb: 'E2E8F0' } } }; });
